...
class Ball2DTracker(Tracker):
  ...
  def __init__(self, algorithm):
    ...
    self.threshold_filter = ThresholdFilter( np.array([20, 150, 50], dtype=np.uint8), np.array([40, 255, 255], dtype=np.uint8) )
    self.algo = algorithm()
  ...
  def captureImageFrame(self):
    ...
    if self.valid:
      ...
      self.transformed_image = cv2.erode(self.transformed_image, K)
      self.transformed_image = cv2.dilate(self.transformed_image, K)
      ...
      ds_centroid = self.centroid
      self.centroid = tuple([2*i for i in self.centroid])
      cv2.circle(self.transformed_image, ds_centroid, 2, (255, 0, 0), -1)
      cv2.circle(self.image, self.centroid, 2, (255, 0, 0), -1)

if __name__ == '__main__':
  # Ball Tracker Tests
  b2dt = Ball2DTracker(Centroid)
